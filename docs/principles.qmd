...
df <- data.frame(
    ...
    value=as.vector(EuStockMarkets)
)

# Add leading spaces for better plotting/labeling.
df$index2 <- paste(" ", df$index)
...
gg <- gg + geom_text(data=y, hjust="left") # aes as before
gg <- gg + theme(legend.position="none")
gg <- gg + xlim(range(pretty(df$time, n=50)))
print(gg)
```
