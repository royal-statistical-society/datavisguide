...
Further information on specific image sizes for different RSS publications is given in the next section. 

As an example, suppose you were creating a plot for the Features section of Significance magazine, and you wanted the plot to span two of the three columns. From the table below, the width of the image should be 124mm. To use the `pdf()` function to save an image, the width and height should be in inches (124mm ~ 4.88in). If we want an 2:1 aspect ratio, we make the height equal to half the width:
